...
class Database:
    def __init__(self):
        self.connection_string = os.environ.get('DATABASE_URL', 'postgresql://localhost/joinerbot')
    ...
    def get_callers(self) -> List[tuple]:
        conn = self._get_connection()
        with conn.cursor() as cur:
            cur.execute("SELECT user_id, username, joined_at FROM callers ORDER BY joined_at")
            return cur.fetchall()
