...
class TestBatchingAndRejoinLogic(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_first_person_starts_batch_timer(self, mock_logger):
        ...
        if Message._batch_timer is not None:
    ...
    async def test_second_person_added_to_batch(self, mock_logger):
        ...
        self.assertEqual(len(Message._pending_joins), 2)
    ...
    @patch("message.Message._logger")
    async def test_batch_notification_deletes_previous_message(self, mock_logger):
        ...
        Message._pending_joins = [(123, "Alice", None)]
    ...
    async def test_suppressed_notification_updates_existing_message(self, mock_logger):
        ...
        Message.set_channel(mock_channel)
        Message._last_message = mock_existing_message
        member_list = [(123, "Alice", None)]
        
        await Message.create(member_list, 1, is_first_person=True, suppress_notification=True)
        
        # Should update existing message but not send new notification
        mock_existing_message.edit.assert_called_once_with(content="Alice joined voice chat")
        mock_channel.send.assert_not_called()
        self.assertIsNone(Message._batch_timer)
    ...
    @patch("message.Message._logger")
    async def test_duplicate_users_removed_from_batch(self, mock_logger):
        ...
        # Should have called send once with deduplicated users
        mock_channel.send.assert_called_once()
        call_args = mock_channel.send.call_args[0][0]
        
        # Check that message contains unique users
        self.assertIn("Alice", call_args)
        self.assertIn("Bob", call_args) 
        self.assertIn("Charlie", call_args)
        # Should only appear once each
        self.assertEqual(call_args.count("Alice"), 1)
        self.assertEqual(len(Message._pending_joins), 0)  # Should be cleared

    @patch("time.time")
    @patch("asyncio.create_task")
    @patch("message.Message._logger")
    async def test_queued_message_for_joins_within_10_minutes(self, mock_logger, mock_create_task, mock_time):
    ...
    async def test_delete_cancels_batch_timer(self, mock_logger):
        ...
        Message._batch_timer = mock_batch_timer
        Message._pending_joins = [(123, "Alice", None)]
